...
export class SqliteReaderFallback {
  ...
  getMessages(sessionIds: string[], range: TimeRangeFilter): RawMessage[] {
    ...
    try {
      ...
      const params = [...sessionIds, fromMs, toMs];
      stmt.bind(params);

      const messages: RawMessage[] = [];

      while (stmt.step()) {
        const row = stmt.getAsObject() as { id: string; session_id: string; time_created: number; data: string };
        try {
  ...
  getParts(messageIds: string[]): RawPart[] {
    ...
    try {
      ...
      stmt.bind(messageIds);

      const parts: RawPart[] = [];

      while (stmt.step()) {
        const row = stmt.getAsObject() as { id: string; message_id: string; session_id: string; data: string };
        try {
          const d = JSON.parse(row.data as string);
